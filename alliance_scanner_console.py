...
def load_scan_configuration(config: dict) -> dict:
    ...
    scan_config['alliance'] = questionary.text(
        message="Alliance name (used for file name):",
        default=config["scan"]["kingdom_name"]
    ).unsafe_ask()

    validated_name = sanitize_scanname(scan_config['alliance'])
...
def safe_scan_execution(alliance_scanner: AllianceScanner, **scan_params) -> None:
    """Execute scan with proper error handling."""
    try:
        alliance_scanner.start_scan(**scan_params)
...
def main():
    logger.info("Starting alliance scanner console application")
    ...
    try:
        logger.info("Initializing AllianceScanner")
        alliance_scanner = AllianceScanner(scan_config['port'], config)
...
if __name__ == "__main__":
    main()
    logger.info("Alliance scanner console application finished")
